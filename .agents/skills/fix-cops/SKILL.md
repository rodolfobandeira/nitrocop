...
5. Regenerate benchmark conformance artifacts only when explicitly requested by the user:
   ...
   ```
   This updates `bench/conform.json` and `bench/results.md` only. It does NOT
   regenerate the generated conformance section in `README.md` or
   `docs/corpus.md`. That generated content must never be edited directly; if
   the user asks for refreshed docs, use the corpus-report generation path
   instead.
